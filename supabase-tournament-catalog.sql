...
-- Provider events are shared across leagues. League-specific state remains on
-- draft_sessions and its draft_teams/draft_picks children.
...
create index if not exists draft_sessions_tournament_idx on public.draft_sessions(tournament_id);
...
order by
  case when draft_sessions.event_id like 'dg:%' then 'data-golf' else 'espn' end,
  draft_sessions.event_id,
  draft_sessions.updated_at desc
...
where draft_sessions.tournament_id is null
  ...
  and tournament_catalog.provider = case
    when draft_sessions.event_id like 'dg:%' then 'data-golf'
    else 'espn'
  end;
