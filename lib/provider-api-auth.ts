...
import { decideProviderCredential, providerResponseHeaders, quotaAllowsProviderCall } from "@/lib/provider-api-policy";
...
export function internalProviderHeaders(): Record<string, string> {
  const key = process.env.INTERNAL_PROVIDER_API_SECRET;
...
export async function authorizeProviderApi(request: NextRequest, route: string, requestsPerMinute: number): Promise<ProviderApiAccess> {
  ...
  const internalSecret = process.env.INTERNAL_PROVIDER_API_SECRET;
  const internalKey = request.headers.get("x-rrg-internal-key") ?? "";
  const decision = decideProviderCredential({
    configured: Boolean(supabaseUrl && serviceRoleKey),
    internalCredentialMatches: Boolean(internalKey && internalSecret && safeSecretEqual(internalKey, internalSecret)),
  ...
  if (decision.kind === "internal") return { ok: true, userId: null, internal: true };
  ...
  const { data: { user }, error: userError } = await supabase.auth.getUser(decision.token);
  if (userError || !user) {
  ...
  if (limitError) {
    console.error("Provider API rate limit check failed", { code: limitError.code });
    return { ok: false, response: NextResponse.json({ ok: false, error: "Provider API rate limiting is unavailable." }, { status: 503 }) };
  }
  if (!allowed) {
    return { ok: false, response: NextResponse.json({ ok: false, error: "Too many provider requests. Please wait a minute and try again." }, { status: 429, headers: { "Retry-After": "60" } }) };
  }
  return { ok: true, userId: user.id, internal: false };
}

export function privateProviderResponse(response: NextResponse) {
  for (const [name, value] of Object.entries(providerResponseHeaders())) response.headers.set(name, value);
  return response;
}
...
export async function consumeProviderQuota(provider: string, requestLimit: number) {
  ...
  if (error) console.error("Provider quota check failed", { code: error.code });
  return quotaAllowsProviderCall({ allowed, error });
}
