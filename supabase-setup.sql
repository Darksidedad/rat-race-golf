...
create or replace function public.consume_provider_rate_limit(
...
declare
  next_count integer;
begin
  if nullif(btrim(target_provider), '') is null or request_limit < 1 or window_seconds < 1 then
    return false;
  end if;
  ...
  set window_start = case
        when provider_rate_limits.window_start <= now() - make_interval(secs => window_seconds) then now()
        else provider_rate_limits.window_start
      end,
      request_count = case
        when provider_rate_limits.window_start <= now() - make_interval(secs => window_seconds) then 1
        else provider_rate_limits.request_count + 1
      end
  returning request_count into next_count;
...
begin
  ...
  into ordered_team_ids, ordered_team_owners, team_count
  from public.draft_teams
  ...
  next_round_number := ((next_pick_number - 1) / team_count) + 1;
  round_index := ((next_pick_number - 1) % team_count) + 1;
  ...
  if not exists (
    select 1
    from unnest(string_to_array(target_session.player_input, E'\n')) field_line
    where lower(btrim(regexp_replace(field_line, '\s+[+-][0-9]+\s*$', '', 'i'))) = lower(cleaned_player_name)
  ) then raise exception 'That golfer is not in this tournament field'; end if;
  if exists (
    select 1 from public.draft_picks
    where session_id = target_session_id
      and (player_key = cleaned_player_key or lower(player_name) = lower(cleaned_player_name))
  ) then raise exception '% has already been drafted', cleaned_player_name; end if;
  ...
  if target_session.field_locked_at is null then
    update public.draft_sessions set field_locked_at = now() where id = target_session_id;
  end if;
